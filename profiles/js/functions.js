...
async function FillData(uid, mode, completeReload = true) {
  ...
  if (completeReload) {
    ...
    <img class="profiles__cover-country" src="https://osu.ppy.sh/images/flags/${oData.country_code}.png" id="country__flag"> ${oData.username} <div id="user__badge" class="osekai__user-badge"></div>
  ...
  if (Exists(oData.roles)) {
    var role = getRole(oData.role);
    ...
    document.getElementById("user__badge").classList.remove("hidden");
  } else {
    document.getElementById("user__badge").classList.add("hidden");
  }
