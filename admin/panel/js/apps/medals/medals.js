...
function displayMedals() {
    ...
    if (currentGroupBy == "categories") {
        ...
        Object.keys(medalList).forEach(medalSection => {
            ...
            displayMedalList(document.getElementById(`medals__${medalSection.toLowerCase().split(' ').join('-')}-list`), medalList[medalSection]);
        });
    }
    if (currentGroupBy == "none") {
        [...document.getElementsByClassName("collapsable-list")].forEach(list => { list.classList.add("hidden"); list.classList.remove("open"); });
...
function groupMedals(groupby) {
    ...
    clearMedalLists();
    if (currentGroupBy == "categories") {
        [...document.getElementsByClassName("collapsable-list")].forEach(list => { list.classList.remove("hidden"); });
    ...
    if (currentGroupBy == "none") {
        [...document.getElementsByClassName("collapsable-list")].forEach(list => { list.classList.add("hidden"); });
        [...document.querySelectorAll("h2[collapsible-button]")].forEach(button => { button.classList.add("hidden"); });
        if (document.getElementById("medals__none-list").classList.contains("hidden")) document.getElementById("medals__none-list").classList.remove("hidden");
        displayMedalList(document.getElementById("medals__none-list"), medalList);
    }
...
function saveMedal(medal, callback) {
    ...
    xhr.send(
        `strSolutionMods=${medal.Mods}&strNewSolution=${medal.Solution}&strSolutionPackID=${medal.PackID}&strSolutionVideo=${medal.Video}&strFirstAchievedDate=${medal.FirstAchievedDate}&strFirstAchievedId=${medal.FirstAchievedBy}&nMedalId=${medal.MedalID}&bBeatmapLockState=${medal.Locked}&strAdditionDate=${medal.Date}`
    );
}

function saveMedalBeatmaps(medal) {
    if (medal.DeletedBeatmaps.length > medalList.find(m => m.MedalID == medal.MedalID).DeletedBeatmaps) {
        ...
        let newDeletedBeatmaps = medal.DeletedBeatmaps.filter(db => !medalList.find(m => m.MedalID == medal.MedalID).DeletedBeatmaps.includes(db));
        newDeletedBeatmaps.forEach(db => {
            saveDeleteBeatmap(db);
        });
    }
    if (medal.Beatmaps.length > medalList.find(m => m.MedalID == medal.MedalID).Beatmaps) {
        // We know there are new beatmaps.. gotta find what ones are added and call the "saveBeatmap" function for each one.
        let newBeatmaps = medal.Beatmaps.filter(b => !medalList.find(m => m.MedalID == medal.MedalID).Beatmaps.includes(b));
...
function saveDeleteBeatmap(beatmapId) {
    ...
    xhr.send(`nBeatmap=${beatmapId}`);
}

function saveRestoreBeatmap(beatmapId) {
...
function displayMedalInformation() {
    ...
    document.getElementsByClassName("medals__medal-info")[0].classList.remove("hidden");
    document.getElementsByClassName("medals__medal-info-title")[0].innerText = medal.Name;
    document.getElementsByClassName("medals__medal-info-description")[0].innerText = medal.Description;
    document.querySelector(".medals__medal-info img").src = medal.Link;
    document.getElementsByClassName("medals__medal-info-solution")[0].innerHTML = medal.Solution != null ? medal.Solution.replaceAll("\n", "<br>") : "";
    ...
    if (medal.PackID != null && medal.PackID != "" && medal.PackID != "0") // Thrill of Chaos uses "0" for some reason.
    {
        ...
        document.getElementById("medals__beatmap-pack-mania").value = packs[2] == undefined ? "" : packs[2];
        document.getElementById("medals__beatmap-pack-catch").value = packs[3] == undefined ? "" : packs[3];
    } else {
...
function displayModSwitches() {
    let modSwitches = document.getElementsByClassName("medals__modswitches")[0];
    colMods.forEach(mod => {
...
function updateMedal() {
    ...
    if (document.getElementById("medals__beatmap-packs").checked == true) {
        medal.PackID = `${document.getElementById("medals__beatmap-pack-osu").value},${document.getElementById("medals__beatmap-pack-taiko").value},${document.getElementById("medals__beatmap-pack-mania").value},${document.getElementById("medals__beatmap-pack-catch").value}`;
    } else {
        medal.PackID = "";
    }
    ...
    saveMedal(medal, function () {
        ...
        document.getElementsByClassName("medals__medal-info-description")[0].innerText = medal.Description;
        document.querySelector(".medals__medal-info img").src = medal.Link;
        document.getElementsByClassName("medals__medal-info-solution")[0].innerHTML = medal.Solution.replaceAll("\n", "<br>");
    });
}
...
function displayBeatmaps() {
        ...
        let liveBeatmapOptions = Object.assign(document.createElement("div"),
            {
                ...
                innerHTML: `<a href="#" id="medals__beatmap-option-delete" class="medals__beatmap-option text-danger" onclick="deleteBeatmap(${beatmap.BeatmapID})">Delete</a><a href="#" id="medals__beatmap-option-edit" class="medals__beatmap-option" onclick="editBeatmap(${beatmap.BeatmapID})">Edit</a>`
            });
        ...
        let beatmapElement = Object.assign(document.createElement("div"),
            {
                ...
                id: "medals__beatmap-" + beatmap.BeatmapID,
    ...
    deletedBeatmaps.forEach(beatmap => {
        let deletedBeatmapOptions = Object.assign(document.createElement("div"),
            {
                className: "medals__beatmap-options",
                innerHTML: `<a href="#" id="medals__beatmap-option-restore" class="medals__beatmap-option text-success" onclick="restoreBeatmap(${beatmap.BeatmapID})">Restore</a><a href="#" id="medals__beatmap-option-edit" class="medals__beatmap-option" onclick="editBeatmap(${beatmap.BeatmapID})">Edit</a>`
            });
        let beatmapDetails = Object.assign(document.createElement("div"),
            {
                className: "medals__beatmap-details",
...
function deleteBeatmap(beatmapID) {
    if (currentMedal.DeletedBeatmaps.find(beatmap => beatmap.BeatmapID == beatmapID) != undefined) {
        console.log("This beatmap has already been deleted.");
        return;
    }
    currentMedal.DeletedBeatmaps.push(currentMedal.Beatmaps.find(beatmap => beatmap.BeatmapID == beatmapID));
    currentMedal.DeletedBeatmaps.find(beatmap => beatmap.BeatmapID == beatmapID).DeletionDate = new Date().toISOString().slice(0, 19).replace('T', ' ');
    currentMedal.Beatmaps = currentMedal.Beatmaps.filter(beatmap => beatmap.BeatmapID != beatmapID);
    displayBeatmaps();
}

function restoreBeatmap(beatmapID) {
    if (currentMedal.Beatmaps.find(beatmap => beatmap.BeatmapID == beatmapID) != undefined) {
        console.log("This beatmap has already been restored.");
        return;
    }
    currentMedal.Beatmaps.push(currentMedal.DeletedBeatmaps.find(beatmap => beatmap.BeatmapID == beatmapID));
    currentMedal.DeletedBeatmaps = currentMedal.DeletedBeatmaps.filter(beatmap => beatmap.BeatmapID != beatmapID);
    currentMedal.Beatmaps.find(beatmap => beatmap.BeatmapID == beatmapID).DeletionDate = null;
    displayBeatmaps();
}

function editBeatmap(beatmapID) {
    let beatmap = currentMedal.Beatmaps.find(beatmap => beatmap.BeatmapID == beatmapID);
    if(beatmap == undefined) beatmap = currentMedal.DeletedBeatmaps.find(beatmap => beatmap.BeatmapID == beatmapID);
...
document.getElementById("medals__searchbar").addEventListener("input", function () { 
    let medalListDivs = document.getElementsByClassName("basic-page-item-list")[0].getElementsByClassName("detailed-list-item");
    ...
    [].forEach.call(medalListDivs, listDiv => {
        ...
        if (!listDiv.getElementsByClassName("item-top")[0].getElementsByClassName("item-texts")[0].getElementsByClassName("item-name")[0].innerText.toLowerCase().includes(medalSearchQuery)) {
            listDiv.classList.add("hidden");
        }
    });
});

// Could move this to document.onload or similar.. 
// or if there's a quicker way to make this load on page load, that'd be great. ~ Coppertine
...
// TODO: Move this out to a global javascript file.. want to make this copyable to other pages.

// ? Tanza/Hubz: i feel like this is something we can get away with with developing on a case-by-case basis
let collapsibleButton = document.querySelectorAll("[collapsible-button]");
...
beatmapPacksCheckbox.addEventListener("click", function handleClick() {
    if (beatmapPacksCheckbox.checked) {
        document.getElementsByClassName("medals__beatmap-packs-ids")[0].classList.add("open");
        ...
        document.getElementsByClassName("medals__beatmap-packs-ids")[0].classList.remove("open");
